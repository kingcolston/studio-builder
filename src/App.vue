  ...
  setup() {
    ...
    const onConnectionCreated = (connection) => {
      ...
      if (connectionId) {
        ...
        if (sourceNode) {
          if (sourceNode.nodeType === 'conditional' && connection.connectionType) {
            // For conditional nodes, update the truePath or falsePath in the config
            const updatedConfig = { ...sourceNode.config };
            
            if (connection.connectionType === 'true') {
              updatedConfig.truePath = connection.targetId;
            } else if (connection.connectionType === 'false') {
              updatedConfig.falsePath = connection.targetId;
            }
            ...
            // For other node types, update the next property
            nodeStore.updateNode(sourceNode.id, {
    ...
    const onConnectionRemoved = (connectionId) => {
      ...
      if (connection) {
        ...
        if (sourceNode) {
          if (sourceNode.nodeType === 'conditional' && connection.connectionType) {
            // For conditional nodes, clear the truePath or falsePath in the config
            const updatedConfig = { ...sourceNode.config };
            
            if (connection.connectionType === 'true' && updatedConfig.truePath === connection.targetId) {
              updatedConfig.truePath = null;
              nodeStore.updateNode(sourceNode.id, {
                config: updatedConfig
              });
            } else if (connection.connectionType === 'false' && updatedConfig.falsePath === connection.targetId) {
              updatedConfig.falsePath = null;
              nodeStore.updateNode(sourceNode.id, {
                config: updatedConfig
              });
            }
          } else if (sourceNode.next === connection.targetId) {
            // For other node types, clear the next property
            nodeStore.updateNode(sourceNode.id, {
              next: null
            });
          }
    ...
    const updateNodeConfigFromForm = (updates) => {
      if (selectedNode.value) {
        ...
        if (selectedNode.value.nodeType === 'conditional') {
          // Handle true/false connections for conditional nodes
          ...
          // Add true path connection if specified
          if (updates.config.truePath) {
            nodeStore.addConnection({
              id: uuidv4(),
              sourceId: selectedNode.value.id,
              targetId: updates.config.truePath,
              connectionType: 'true'
            });
          }
          
          // Add false path connection if specified
          if (updates.config.falsePath) {
            nodeStore.addConnection({
              id: uuidv4(),
              sourceId: selectedNode.value.id,
              targetId: updates.config.falsePath,
              connectionType: 'false'
            });
          }
        } else {
          // Handle standard next node connections for other node types
    ...
    // Method to import a flow from a JSON file
    const importFlowFromJson = (event) => {
      ...
      reader.onload = (e) => {
        try {
          ...
          if (flowId) {
            // Set the imported flow as the active flow
            flowStore.setActiveFlow(flowId);
            
            // Show a success message
            alert(`Flow "${flowStore.flowById(flowId).name}" imported successfully!`);
          } else {
